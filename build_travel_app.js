...
function dashboardStrings(){
 return state.lang === 'cs' ? {
  title:'Dnešní plán', liveFallback:'Živé počasí není dostupné — doporučení vychází z obtížnosti trasy a vhodnosti pro rodinu.', temp:'Teplota', rain:'Déšť', wind:'Vítr', sunset:'Západ', thunder:'Bouřky', hourly:'Hodinová předpověď', showHourly:'Zobrazit hodiny', hideHourly:'Skrýt hodiny', hottest:'Nejtepleji', wettest:'Nejvíc deště', rainChance:'Šance deště', rainAmount:'Srážky', recommend:'Doporučená túra', reason:'Proč dnes', depart:'Odjezd', return:'Návrat', swim:'Koupání po túře', energy:'Rodinná energie', great:'Skvělá', normal:'Normální', tired:'Unavení', start:'Začít dnešní plán', nav:'Navigovat na parkování', details:'Otevřít detail túry', pack:'Co sbalit', timeline:'Dnešní harmonogram', emergency:'Nouzové info', call:'Volat 112', close:'Zavřít', gps:'Aktuální GPS', gpsAsk:'Získat GPS polohu', gpsUnknown:'GPS zatím není k dispozici', accommodation:'Ubytování', nearestParking:'Parkování dnešní túry', danger:'Pokud jste v nebezpečí, volejte 112 a sdílejte svou GPS polohu.', swimYes:'Ano, vezměte plavky a ručníky.', swimMaybe:'Možná, podle počasí a energie dětí.', swimNo:'Dnes raději bez koupání.', statusGood:'Dobré', statusOk:'Opatrně', statusBad:'Nevhodné'
 } : {
  title:'Vacation Dashboard', liveFallback:'Live weather unavailable — recommendations are based on route difficulty and family suitability.', temp:'Temperature', rain:'Rain', wind:'Wind', sunset:'Sunset', thunder:'Storms', hourly:'Hourly forecast', showHourly:'Show hours', hideHourly:'Hide hours', hottest:'Hottest', wettest:'Most rain', rainChance:'Rain chance', rainAmount:'Rain amount', recommend:'Recommended hike', reason:'Why today', depart:'Depart', return:'Return', swim:'Swim after hike', energy:'Family energy', great:'Great', normal:'Normal', tired:'Tired', start:'Start today’s plan', nav:'Navigate to parking', details:'Open hike details', pack:'What to pack', timeline:'Today’s timeline', emergency:'Emergency', call:'Call 112', close:'Close', gps:'Current GPS', gpsAsk:'Get GPS location', gpsUnknown:'GPS not available yet', accommodation:'Accommodation', nearestParking:'Today’s nearest parking', danger:'If in danger, call 112 and share your GPS location.', swimYes:'Yes, pack swimwear and towels.', swimMaybe:'Maybe, depending on weather and kids’ energy.', swimNo:'Skip swimming today.', statusGood:'Good', statusOk:'Caution', statusBad:'Avoid'
 };
}
function staticWeather(){ return { live:false, temp:null, rain:null, wind:null, thunder:false, sunset:null, code:null, hourly:[] }; }
async function loadWeather(){
 try{
  const url='https://api.open-meteo.com/v1/forecast?latitude=46.278&longitude=13.835&current=temperature_2m,weather_code,wind_speed_10m&hourly=temperature_2m,precipitation_probability,precipitation,weather_code,wind_speed_10m&daily=precipitation_probability_max,sunset,weather_code&timezone=Europe%2FPrague&forecast_days=1';
  const r=await fetch(url);
  if(!r.ok) throw new Error('weather '+r.status);
  const d=await r.json();
  const hourly=(d.hourly?.time || []).map(function(time,i){ return { time, temp:Math.round(d.hourly.temperature_2m[i]), rain:d.hourly.precipitation_probability[i] ?? 0, precip:d.hourly.precipitation[i] ?? 0, code:d.hourly.weather_code[i] ?? 0, wind:Math.round(d.hourly.wind_speed_10m[i] ?? 0), thunder:(d.hourly.weather_code[i] ?? 0) >= 95 }; });
  state.weather={ live:true, temp:Math.round(d.current.temperature_2m), rain:d.daily.precipitation_probability_max[0] || 0, wind:Math.round(d.current.wind_speed_10m || 0), thunder:(d.daily.weather_code[0] || d.current.weather_code || 0) >= 95, sunset:d.daily.sunset[0], code:d.current.weather_code, hourly };
...
function hourlyWindow(){
 const w=state.weather || staticWeather();
 if(!w.live || !w.hourly?.length) return [];
 const now=new Date();
 return w.hourly.map(h=>({...h, date:new Date(h.time)})).filter(h=>h.date.getHours()>=6 && h.date.getHours()<=22);
}
function hourlyForecastHtml(){
 const L=dashboardStrings(); const hours=hourlyWindow();
 if(!hours.length) return '<p class="weather">'+L.liveFallback+'</p>';
 const hottest=hours.reduce((a,b)=>b.temp>a.temp?b:a,hours[0]);
 const wettest=hours.reduce((a,b)=>((b.rain||0)>(a.rain||0)?b:a),hours[0]);
 const cards=hours.map(function(h){ const cls=h.thunder?'storm-hour':(h.rain>=45?'rain-high':''); return '<div class="hour-card '+cls+'"><div class="time">'+fmtTime(h.date)+'</div><i class="fa-solid '+weatherIcon(h.code,h.thunder)+'"></i><b>'+h.temp+'°C</b><span>'+L.rainChance+' '+h.rain+'%</span><span>'+L.rainAmount+' '+h.precip+' mm</span><span>'+h.wind+' km/h</span></div>'; }).join('');
 return '<div class="forecast-summary"><div class="dash-metric"><span>'+L.hottest+'</span><b>'+fmtTime(hottest.date)+' · '+hottest.temp+'°C</b></div><div class="dash-metric"><span>'+L.wettest+'</span><b>'+fmtTime(wettest.date)+' · '+wettest.rain+'%</b></div></div><div class="hourly-strip">'+cards+'</div>';
}
...
document.addEventListener('click', async e => {
 ...
 if(e.target.closest('[data-toggle-forecast]')) { const panel=$('#hourlyForecast'); const label=$('[data-forecast-label]'); const L=dashboardStrings(); panel?.classList.toggle('active'); if(label) label.textContent=panel?.classList.contains('active') ? L.hideHourly : L.showHourly; }
...
function makeServiceWorker() {
  ...
  return `const CACHE='bohinj-family-hikes-v8-hourly-weather';
